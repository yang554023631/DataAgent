...
为了在效果和延迟之间取得平衡，NL→DSL 管线采用双模型配置：

| 环节 | 模型 | 说明 |
|------|------|------|
| 查询规划 | 轻量模型（doubao-seed-2.0-lite） | 任务格式明确，输出 JSON 结构固定，轻量模型可胜任 |
| DSL 生成 | 轻量模型（doubao-seed-2.0-lite） | 输出格式相对固定（ES DSL JSON），轻量模型 + 完整 prompt 可保证质量 |
| 反思修正 | 轻量模型（doubao-seed-2.0-lite） | 同 DSL 生成，改错也是格式明确的任务 |

**配置方式**：
- 主模型：`ARK_LLM_MODEL` 环境变量（用于意图识别、RAG 问答等通用任务）
- 轻量模型：`ARK_LITE_MODEL` 环境变量（默认值：`ep-20260807194303-xq99t`）
- 均通过火山引擎 Ark API（OpenAI 兼容协议）调用，共用 `ARK_API_KEY` 和 `ARK_BASE_URL`

**降级策略**：未配置轻量模型时，自动降级使用主模型。

**性能收益**：相比全主模型方案，NL→DSL 总耗时降低约 20-25%。
- 查询规划：~20s → ~14s
- DSL 生成：~33s → ~27s
- nl_dsl 节点总计：~53s → ~41s

**质量注意事项**：
- 轻量模型输出规范性略低于主模型（如变量命名风格不一致），但不影响功能
- DSL 正确性由安全校验层 + 执行层双层保障，输出格式问题会被捕获并触发反思重试
  ↓
Step 3: 结果汇总 + 格式化
  ↓
统一格式输出
