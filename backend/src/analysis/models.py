...
def coerce_to_str(value: Any) -> str:
    """Coerce any value (including int/float) to string for Pydantic validation."""
    if value is None:
        return value
    return str(value)

def coerce_list_to_str(values: List[Any]) -> List[str]:
    """Coerce all items in list to string."""
    return [coerce_to_str(v) for v in values]
...
class FieldContext(BaseModel):
    ...
    advertiser_ids: Optional[Annotated[List[str], BeforeValidator(coerce_list_to_str)]] = Field(None, description="Advertiser IDs (can be string UUID or integer)")
